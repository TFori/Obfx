...
def bytecode_to_opcode(bytecode):
    ...
    while i < len(bytecode):
        ...
        if opcodes_dict[opcode][0] == PUSH:
            push_offset = int(bytecode[i],16) - int("60",16) +1  #(0x60 to 0x7f) #get the number of bytes next to PUSH instruction
            value = "".join(bytecode[i+1:i+push_offset+1]) #merge the bytes next to PUSH instruction
            opcodes_obj_list.append(PUSH(push_offset, value)) #create PUSH object
            i+=push_offset

        elif opcodes_dict[opcode][0] == DUP:
            stack_position = int(bytecode[i],16) - int("80",16) #(0x80 to 0x8f) #get the number of bytes next to DUP instruction

            opcodes_obj_list.append(DUP(stack_position)) #create DUP object

        elif opcodes_dict[opcode][0] == SWAP:
            stack_position = int(bytecode[i],16) - int("90",16)
